"""OpenAI-compatible Chat Completions transport via aiohttp."""
...
def _fit_output_limit(payload: dict[str, Any], model: ModelSpec) -> int:
    ...
    prompt = json.dumps(
        [payload.get("instructions"), payload.get("messages", payload.get("input")), payload.get("tools")],
        ensure_ascii=False,
    )
...
class _OpenAIHTTPTransport(CompletionTransport, EmbeddingTransport):
            ...
            try:
                async with self.session.post(url, json=payload, headers=headers) as resp:
                    ...
                    if resp.status == 429 or resp.status >= 500:
                        retry_resp = resp
                        last_exc = StreamError(f"OpenAI-compatible API error {resp.status}: {body}")
                        logger.warning(
                        ...
                        raise StreamError(f"OpenAI-compatible API error {resp.status}: {body}")
    ...
    async def fetch_models(self) -> None:
        self.models = ModelRegistry(OPENAI_MODELS.values())
...
@dataclass(slots=True)
class ChatCompletionsTransport(_OpenAIHTTPTransport):
