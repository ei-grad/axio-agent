...
| `sandbox.exec_stream(command, timeout=30, stdin=None)` | Async iterator yielding `(stdout_or_stderr, text)` chunks in Docker's observed multiplex-frame order. |
...
`exec_stream()` and the final `exec()` string retain Docker's observed
multiplex-frame order. The final string labels each stdout/stderr transition;
its initial stdout segment remains unlabelled. This does not establish a global
ordering for writes to independent file descriptors inside the container.
